...
End-to-end pipeline that takes the current feature branch from "code written" to "deployed on staging". Each step is a separate bash script with emoji output. Codex orchestrates: confirms ops with user, runs scripts, monitors output, makes flexible decisions only when something fails or needs judgment.
...
This is where Codex makes judgment calls. Default is to **try to fix automatically**, then report what was done.
...
All scripts live in `scripts/` and are self-contained — Codex shells out to them. Each prints emoji-prefixed status lines.

- `00-preflight.sh` — branch/state checks
- `01-code-review.sh` — diff dump for code review
- `02-format.sh` — `turbo format`
- `03-typecheck.sh` — `turbo typecheck`
- `04-test.sh` — `turbo test`
- `05-create-pr.sh` — `gh pr create` to dev (idempotent)
- `06-resolve-dev.sh` — rebase/merge `origin/dev`
- `07-monitor-ci.sh` — `gh pr checks --watch`
- `08-pull-staging.sh` — fetch + checkout staging + pull
- `09-merge-to-staging.sh` — merge feature branch
- `10-push-staging.sh` — push staging
- `11-verify-deploy.sh` — ArgoCD + kubectl + health check

Run with `bash .Codex/skills/ship-to-staging/scripts/<name>.sh [args]`.

## Notes

- **Never** force-push `staging` or `main`. If `09-merge` fails non-trivially, stop and ask.
- **Never** add `Co-Authored-By:` trailers (AGENTS.md rule — Vercel Hobby blocks these).
