...
**Module:** `<module>` — take the module name from the user's request (or the upstream skill that invoked this one).
...
- **Canonical examples (co-located, generated):**
  `apps/api/src/modules/cv-adapter/cv-adapter.ddd.yaml` and
  `apps/api/src/modules/sales-os/presale.ddd.yaml`. Mirror their shape (flat
...
- **DDD module id vs source directory may differ.** The `module:` field is the bounded-context
  id and can differ from the folder, e.g. the **Presale** context lives in the `sales-os`
  directory as `sales-os/presale.ddd.yaml` (`module: presale`). The assembler keys by the
  `module:` field, not the filename, and globs `apps/api/**/*.ddd.yaml` — so the filename is
  free. The `hash` helper names the hashes file after the **directory** (`sales-os.hashes.yaml`).
  A bounded context can also span dirs (Presale uses `sales-os` + the separate `cv-adapter`).
...
**Why this is non-negotiable** (real finding from the presale pass): a workflow engine
