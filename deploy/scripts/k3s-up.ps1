...
$KubeConfig = "$env:USERPROFILE\.kube\k3s-config"

# -- helpers ---------------------------------------------------------------
function wsl-run($cmd) { wsl -d $WslDistro -u root -e bash -c $cmd 2>&1 }
function kubectl-run { & kubectl --kubeconfig=$KubeConfig $args }
...
if ($k3sRunning.Trim() -eq "0") {
    ...
    # Start k3s via setsid so it survives the bash -c exit
    Write-Output "  Starting k3s server (via setsid)..."
    wsl-run "setsid /usr/local/bin/k3s server --write-kubeconfig-mode 644 > /var/log/k3s.log 2>&1 &"
    ...
    # Set up auto-start on WSL boot if requested or first install
    if ($AutoStartWsl -or $k3sBin.Trim() -eq "missing") {
        Write-Output "  Setting up auto-start on WSL boot..."
        wsl-run @"
grep -q "k3s server" /etc/wsl.conf 2>/dev/null && exit 0
# If [boot] section exists, append command line; otherwise create section
if grep -q '^\[boot\]' /etc/wsl.conf; then
  sed -i '/^\[boot\]/a command = /bin/bash -c '"'"'/usr/local/bin/k3s server --write-kubeconfig-mode 644 > /var/log/k3s.log 2>&1 &'"'"'' /etc/wsl.conf
else
  printf '[boot]\ncommand = /bin/bash -c '"'"'/usr/local/bin/k3s server --write-kubeconfig-mode 644 > /var/log/k3s.log 2>&1 &'"'"'\n' >> /etc/wsl.conf
fi
"@
    }
}

# -- Step 1: copy kubeconfig to Windows (direct WSL2 IP) --------------------
Write-Output "=== Step 1: Set up kubeconfig ==="
$wslIp = (wsl-run "hostname -I" | Out-String).Trim().Split()[0]
...
Set-Content -Path $KubeConfig -Value $kubeconfig
...
kubectl-run create namespace $Namespace --dry-run=client -o yaml | kubectl-run apply -f -

# -- Step 5: Create API secret ----------------------------------------------
Write-Output "=== Step 5: Create API secret ==="
kubectl-run create secret generic $SecretName --namespace=$Namespace --dry-run=client -o yaml `
...
kubectl-run wait --for=condition=Available deployment -l app.kubernetes.io/name=api -n $Namespace --timeout=120s 2>$null
kubectl-run wait --for=condition=Available deployment -l app.kubernetes.io/component=worker -n $Namespace --timeout=120s 2>$null

# -- Step 8: Port-forward API -----------------------------------------------
Write-Output "=== Step 8: Port-forward API ==="
Write-Output "  API available at http://localhost:3000/health"
kubectl-run port-forward -n $Namespace svc/trendscout-api 3000:3000
